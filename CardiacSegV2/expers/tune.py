...
def simple_boundary_loss(pred, target):
    """
    pred:   [B, 1, D, H, W] softmax 後的機率 (myocardium)
    target: [B, 1, D, H, W] binary GT (myocardium mask)
    """

    # Sobel 3D edge kernel (用來抓邊界)
    ...
    ).unsqueeze(0).unsqueeze(0)  # shape [1,1,3,3,3]

    # gradient magnitude = edges
    pred_edge = torch.abs(F.conv3d(pred, sobel_x, padding=1))
    target_edge = torch.abs(F.conv3d(target, sobel_x, padding=1))
...
def main_worker(args):
    ...
    if args.loss == 'dice_focal_loss':
        ...
        print("loss: dice focal + boundary loss") 
        # segmentation base loss 
        ...
        MYO_CLASS = 1 # ⭐ 根據你資料，1 比較可能是心肌 
        lambda_boundary = 0.2 
        def dice_loss(logits, labels): 
            seg_loss = base_loss(logits, labels) 
            # myocardium channel 機率 
            probs = torch.softmax(logits, dim=1) 
            myo_pred = probs[:, MYO_CLASS:MYO_CLASS+1] 
            
            # labels -> myocardium binary 
            if labels.dim() == 5 and labels.size(1) == 1: 
                gt = labels[:, 0] 
            else: 
                gt = labels 
            
            myo_target = (gt == MYO_CLASS).float().unsqueeze(1) 
            
            # boundary loss 
            b_loss = simple_boundary_loss(myo_pred, myo_target) 
            
            return seg_loss + lambda_boundary * b_loss
    

    elif args.loss == "boundary_iou_loss":
        print("loss: (DiceFocal(all classes) + boundary(MYO) + IoU(MYO)) / 2")
        MYO_CLASS = 1          # 心肌類別
        lambda_boundary = 0.2  
        eps = 1e-6
        ...
        def dice_loss(logits, labels):
            base_seg_loss = base_loss(logits, labels)   # scalar
            ...
            # boundary loss (只針對 MYO) 
            b_loss = simple_boundary_loss(myo_pred, myo_target)
        
            # IoU loss (只針對 MYO) 
            intersection = (myo_pred * myo_target).sum()
            denom = myo_pred.sum() + myo_target.sum()
            union = denom - intersection
            iou = (intersection + eps) / (union + eps)
            iou_loss = 1.0 - iou  
        
            seg_with_boundary = base_seg_loss + lambda_boundary * b_loss
        
            total_loss = 0.5 * (seg_with_boundary + iou_loss)
